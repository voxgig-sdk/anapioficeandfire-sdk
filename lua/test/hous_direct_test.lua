...
describe("HousDirect", function()
  it("should direct-list-hous", function()
    local setup = hous_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-hous", setup.live and "live" or "unit")
  ...
  it("should direct-load-hous", function()
    local setup = hous_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-hous", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function hous_direct_setup(mockres)
