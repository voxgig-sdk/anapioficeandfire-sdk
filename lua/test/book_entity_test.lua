...
describe("BookEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set ANAPIOFICEANDFIRE_TEST_BOOK_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    assert.is_not_nil(book_ref01_data_dt0_loaded)
...
function book_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("ANAPIOFICEANDFIRE_TEST_BOOK_ENTID")
