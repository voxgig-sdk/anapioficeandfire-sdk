...
describe("HouseDirect", function()
  it("should direct-list-house", function()
    local setup = house_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-house", setup.live and "live" or "unit")
  ...
  it("should direct-load-house", function()
    local setup = house_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-house", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function house_direct_setup(mockres)
