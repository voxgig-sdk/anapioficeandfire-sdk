...
local HouseEntity = {}
HouseEntity.__index = HouseEntity


function HouseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HouseEntity)
  self._name = "house"
  self._client = client
...
function HouseEntity:get_name()
  return self._name
end


function HouseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HouseEntity.new(self._client, opts)
end
...
function HouseEntity:mark_deleted()
  self._deleted = true
end


function HouseEntity:deleted()
  return true == self._deleted
end


function HouseEntity:data_set(args)
  if args ~= nil then
...
function HouseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HouseEntity:match_set(args)
  if args ~= nil then
...
function HouseEntity:match_get()
...
function HouseEntity:stream(action, args, callopts)
...
function HouseEntity:_run_op(ctx, post_done)
  local utility = self._utility
