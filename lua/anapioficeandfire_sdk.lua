...
require("anapioficeandfire_types")
...
local AnapioficeandfireSDK = {}
AnapioficeandfireSDK.__index = AnapioficeandfireSDK
...
AnapioficeandfireSDK._make_feature = _make_feature


function AnapioficeandfireSDK.new(options)
  local self = setmetatable({}, AnapioficeandfireSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function AnapioficeandfireSDK:options_map()
...
function AnapioficeandfireSDK:get_utility()
  return Utility.copy(self._utility)
end


function AnapioficeandfireSDK:get_root_ctx()
  return self._rootctx
end


function AnapioficeandfireSDK:prepare(fetchargs)
...
function AnapioficeandfireSDK:direct(fetchargs)
...
function AnapioficeandfireSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function AnapioficeandfireSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "AnapioficeandfireSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function AnapioficeandfireSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function AnapioficeandfireSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "AnapioficeandfireSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function AnapioficeandfireSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = AnapioficeandfireSDK.new(sdkopts)
  sdk.mode = "test"
