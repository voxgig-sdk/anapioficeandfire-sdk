...
//
// NewAnapioficeandfireSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.AnapioficeandfireSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewAnapioficeandfireSDK(opts)
}
...
func fcCandidates(client *sdk.AnapioficeandfireSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.AnapioficeandfireSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.AnapioficeandfireSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.AnapioficeandfireError
