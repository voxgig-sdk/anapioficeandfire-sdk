...
module AnapioficeandfireUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AnapioficeandfireResponse.new({})
    ...
    if fetch_err
      ...
      response = AnapioficeandfireResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AnapioficeandfireResponse.new(fetched)
    else
