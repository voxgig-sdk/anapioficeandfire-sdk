...
// Anapioficeandfire SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class AnapioficeandfireTestFeature extends AnapioficeandfireBaseFeature
{
    ...
    public function init(AnapioficeandfireContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (AnapioficeandfireContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = AnapioficeandfireParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (AnapioficeandfireContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(AnapioficeandfireContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = AnapioficeandfireParam::call($ctx, $k);
