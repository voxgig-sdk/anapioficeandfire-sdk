...
// Anapioficeandfire SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = AnapioficeandfireOmni::makeStructRunner(
    ...
    {
        return AnapioficeandfireOmni::objmodel(AnapioficeandfireOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return AnapioficeandfireOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return AnapioficeandfireOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(AnapioficeandfireOmni::structfix($now, Struct::undef()))
                        !== json_encode(AnapioficeandfireOmni::structfix($val, Struct::undef()))) {
