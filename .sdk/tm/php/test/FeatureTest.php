...
// Anapioficeandfire SDK feature test
//
...
class FtCtrl extends AnapioficeandfireControl
{
...
class FtClient
{
    ...
    private AnapioficeandfireUtility $utility;

    public function __construct(AnapioficeandfireUtility $utility, array $options)
    {
    ...
    public function get_utility(): AnapioficeandfireUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AnapioficeandfireContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AnapioficeandfireContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AnapioficeandfireContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AnapioficeandfireConfig::make_feature($name);
    ...
    private static function buildUrl(AnapioficeandfireSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AnapioficeandfireError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AnapioficeandfireResponse($fetched);
        }
    ...
    private function fail(AnapioficeandfireContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AnapioficeandfireContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
