...
//    only - a typed AnapioficeandfireContext there would make every ctx
...
final class AnapioficeandfireOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new AnapioficeandfireContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new AnapioficeandfireSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new AnapioficeandfireResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new AnapioficeandfireError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new AnapioficeandfireResponse($clean['response']);
    ...
    public static function err_from_map($m): ?AnapioficeandfireError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new AnapioficeandfireError(is_string($code) ? $code : '', $msg);
    }
