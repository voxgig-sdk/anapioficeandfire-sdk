...
// Anapioficeandfire SDK pipeline test
//
...
{
    private static function utility(): AnapioficeandfireUtility
    {
        return new AnapioficeandfireUtility();
    }
    ...
    private static function ctx(array $over = []): AnapioficeandfireContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new AnapioficeandfireContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new AnapioficeandfireOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AnapioficeandfireError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AnapioficeandfireOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AnapioficeandfireMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AnapioficeandfireOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AnapioficeandfireMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AnapioficeandfireOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AnapioficeandfireMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AnapioficeandfireSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AnapioficeandfireMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new AnapioficeandfireResponse([]),
            'result' => new AnapioficeandfireResult([]),
        ]);
        [, $err] = AnapioficeandfireMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new AnapioficeandfireSpec([]),
            'response' => null,
            'result' => new AnapioficeandfireResult([]),
        ]);
        [, $err] = AnapioficeandfireMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AnapioficeandfireSpec(['step' => 's']),
            'response' => new AnapioficeandfireResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AnapioficeandfireResult([]),
        ]);
        [, $err] = AnapioficeandfireMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AnapioficeandfireSpec(['step' => 's']),
            'response' => new AnapioficeandfireResponse(self::resp(200, ['v' => 1])),
            'result' => new AnapioficeandfireResult([]),
        ]);
        [, $err] = AnapioficeandfireMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AnapioficeandfireSpec(['step' => 's']),
            'response' => new AnapioficeandfireResponse(self::resp(200, ['v' => 2])),
            'result' => new AnapioficeandfireResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AnapioficeandfireMakeResponse::call($ctx);
    ...
    {
        $preset = new AnapioficeandfireResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AnapioficeandfireSpec([]),
            'response' => new AnapioficeandfireResponse([]),
            'result' => new AnapioficeandfireResult([]),
        ...
        [$got, $err] = AnapioficeandfireMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AnapioficeandfireResult([])]);
        [, $err] = AnapioficeandfireMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AnapioficeandfireSpec([]), 'result' => null]);
        [, $err] = AnapioficeandfireMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AnapioficeandfireSpec(['step' => 's']),
            'result' => new AnapioficeandfireResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AnapioficeandfireOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AnapioficeandfireMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AnapioficeandfireSpec(['step' => 's']),
            'result' => new AnapioficeandfireResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AnapioficeandfireOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AnapioficeandfireMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AnapioficeandfireResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AnapioficeandfireSpec([]),
            'result' => new AnapioficeandfireResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AnapioficeandfireMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AnapioficeandfireMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new AnapioficeandfireError('boom', 'boom');
        $utility->fetcher = function (AnapioficeandfireContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AnapioficeandfireSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AnapioficeandfireMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AnapioficeandfireContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AnapioficeandfireMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AnapioficeandfireResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        AnapioficeandfireMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AnapioficeandfireContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AnapioficeandfireSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AnapioficeandfireMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AnapioficeandfireResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AnapioficeandfireSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AnapioficeandfireMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = AnapioficeandfireMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AnapioficeandfireMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AnapioficeandfireResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AnapioficeandfireDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AnapioficeandfireResult(['ok' => false])]);
        $this->expectException(AnapioficeandfireError::class);
        AnapioficeandfireDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AnapioficeandfireResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AnapioficeandfireMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        AnapioficeandfireMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AnapioficeandfireBaseFeature();
        $b = new AnapioficeandfireBaseFeature();
        AnapioficeandfireFeatureAdd::call($ctx, $a);
        AnapioficeandfireFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AnapioficeandfireBaseFeature
    {
        $f = new AnapioficeandfireBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        AnapioficeandfireFeatureAdd::call($ctx, self::named_feature('a'));
        AnapioficeandfireFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        AnapioficeandfireFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        AnapioficeandfireFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        AnapioficeandfireFeatureAdd::call($ctx, $replace);
        ...
        AnapioficeandfireFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): AnapioficeandfireContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new AnapioficeandfireSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AnapioficeandfirePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        AnapioficeandfirePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new AnapioficeandfireResponse(['headers' => null]),
            'result' => new AnapioficeandfireResult([]),
        ]);
        AnapioficeandfireResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        AnapioficeandfireResultBody::call($ctx);
