...
class HouseEntity
  ...
  def make
    opts = @_entopts.dup
    HouseEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AnapioficeandfireHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [House, Hash] the current House data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AnapioficeandfireHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of House fields)
  def match_get
  ...
  # Load a single House.
  #
  # @param reqmatch [HouseLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.House.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [House, Hash] the loaded House; raises AnapioficeandfireError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AnapioficeandfireHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [HouseListMatch, Hash, nil] match filter (any subset of
  #   House fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<House>, Array] the matching House items; raises AnapioficeandfireError on failure
