...
require_relative 'Anapioficeandfire_types'
...
class AnapioficeandfireSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AnapioficeandfireHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AnapioficeandfireHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AnapioficeandfireFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    AnapioficeandfireUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = AnapioficeandfireHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = AnapioficeandfireHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AnapioficeandfireHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => AnapioficeandfireError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue AnapioficeandfireError => err
    ...
    if fetched.is_a?(Hash)
      status = AnapioficeandfireHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AnapioficeandfireError.new(
        "graphql_error", "AnapioficeandfireSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = AnapioficeandfireSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
