# anapioficeandfire-cli

boru-driven command-line client **and** interactive REPL for the Anapioficeandfire
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/anapioficeandfire-cli)
make build

# 2. See usage (words, entities, env vars)
./anapioficeandfire-cli --help

# 3. Provide credentials once, via the environment
export ANAPIOFICEANDFIRE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./anapioficeandfire-cli list book
./anapioficeandfire-cli load 1 book            # {id:1} shorthand
./anapioficeandfire-cli load '{id:1}' book       # explicit match map
./anapioficeandfire-cli list character

# 5. Override the API base URL for a single call
ANAPIOFICEANDFIRE_BASE=https://api.example.com ./anapioficeandfire-cli list book

# 6. No arguments -> interactive REPL
./anapioficeandfire-cli
anapioficeandfire> list book
anapioficeandfire> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/anapioficeandfire-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/anapioficeandfire-cli list book
   ```
...
```sh
./anapioficeandfire-cli load 1 book          # scalar shorthand for {id:1}
./anapioficeandfire-cli load '{id:1}' book     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `anapioficeandfire>`). Each line is
evaluated as its own boru expression:

```text
$ ./anapioficeandfire-cli
anapioficeandfire> list book
anapioficeandfire> /help
anapioficeandfire> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/anapioficeandfire-cli`. |
...
(`./anapioficeandfire-cli <expr>`) and interactively (the REPL), and expressions compose the
