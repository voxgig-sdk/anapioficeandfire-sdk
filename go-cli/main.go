// Command Anapioficeandfire-cli is a CLI + REPL for the Anapioficeandfire SDK,
...
const prompt = "anapioficeandfire"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "book character house"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ANAPIOFICEANDFIRE_APIKEY (api key), ANAPIOFICEANDFIRE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ANAPIOFICEANDFIRE_APIKEY carries the API key and
	// ANAPIOFICEANDFIRE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ANAPIOFICEANDFIRE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ANAPIOFICEANDFIRE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAnapioficeandfireSDK(opts)
