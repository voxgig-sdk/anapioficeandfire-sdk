# Anapioficeandfire SDK test runner

from __future__ import annotations
import os
import json

from anapioficeandfire_sdk.utility.voxgig_struct import voxgig_struct as vs


class AnapioficeandfireTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AnapioficeandfireTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AnapioficeandfireTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AnapioficeandfireTestRunner.getenv("ANAPIOFICEANDFIRE_TEST_LIVE")
        override = AnapioficeandfireTestRunner.getenv("ANAPIOFICEANDFIRE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AnapioficeandfireTestRunner.getenv(key)
        ...
        explain = AnapioficeandfireTestRunner.getenv("ANAPIOFICEANDFIRE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ANAPIOFICEANDFIRE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if AnapioficeandfireTestRunner._test_control is not None:
            return AnapioficeandfireTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AnapioficeandfireTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AnapioficeandfireTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AnapioficeandfireTestRunner.load_test_control()
...
def load_env_local():
    AnapioficeandfireTestRunner.load_env_local()


def env_override(m):
    return AnapioficeandfireTestRunner.env_override(m)


def entity_data(v):
    return AnapioficeandfireTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AnapioficeandfireTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AnapioficeandfireTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AnapioficeandfireTestRunner.load_test_control()


def live_client_options():
    return AnapioficeandfireTestRunner.live_client_options()


def live_delay_ms():
    return AnapioficeandfireTestRunner.live_delay_ms()
