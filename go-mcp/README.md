...
MCP server exposing the Anapioficeandfire SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `anapioficeandfire_list` | `entity`, optional `query` map | First page of records as JSON |
| `anapioficeandfire_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `anapioficeandfire_list` and
`anapioficeandfire_load` tools become available in new sessions.
