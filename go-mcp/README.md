...
[MCP](https://modelcontextprotocol.io) server exposing the Anapioficeandfire SDK as
two agent tools — `anapioficeandfire_list` and `anapioficeandfire_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/anapioficeandfire-mcp)
make build

# 2. Provide credentials via the environment
export ANAPIOFICEANDFIRE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user anapioficeandfire \
  -- /absolute/path/to/anapioficeandfire-mcp -transport stdio
...
```jsonc
// anapioficeandfire_list: first page of records
{ "entity": "book" }
{ "entity": "book", "query": { } }

// anapioficeandfire_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/anapioficeandfire-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user anapioficeandfire \
     -- "$PWD"/dist/*/anapioficeandfire-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `anapioficeandfire_list` and `anapioficeandfire_load` tools now appear
   in new sessions. Ask the agent to *"list book using anapioficeandfire"*
   and it calls `anapioficeandfire_list` with `{"entity":"book"}`.
...
```sh
export ANAPIOFICEANDFIRE_APIKEY=sk_live_xxx            # API key
export ANAPIOFICEANDFIRE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `anapioficeandfire_list` tool
...
### Call the `anapioficeandfire_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `anapioficeandfire_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `anapioficeandfire_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ANAPIOFICEANDFIRE_APIKEY` | API key sent with every request. |
| `ANAPIOFICEANDFIRE_BASE` | Optional override of the API base URL. |
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"anapioficeandfire_load","arguments":{"entity":"book","query":{"id":1}}}}'
```
