...
// Anapioficeandfire SDK retry feature
...
class AnapioficeandfireRetryFeature extends AnapioficeandfireBaseFeature
{
    ...
    public function init(AnapioficeandfireContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AnapioficeandfireContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AnapioficeandfireContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AnapioficeandfireContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
