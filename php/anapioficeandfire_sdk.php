...
class AnapioficeandfireSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = AnapioficeandfireHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = AnapioficeandfireHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!AnapioficeandfireFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, AnapioficeandfireFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return AnapioficeandfireUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = AnapioficeandfireHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = AnapioficeandfireHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = AnapioficeandfireHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new AnapioficeandfireError($op . "_allow",
                "AnapioficeandfireSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = AnapioficeandfireHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new AnapioficeandfireError("graphql_error",
                "AnapioficeandfireSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function House($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_house === null) {
                $this->_house = new HouseEntity($this, null);
            }
            return $this->_house;
        }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new AnapioficeandfireSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
