...
describe('HouseDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new AnapioficeandfireSDK({
  ...
  test('direct-load-house', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-house', setup.live)) return
  ...
  test('direct-list-house', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-house', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ANAPIOFICEANDFIRE_TEST_HOUSE_ENTID']
