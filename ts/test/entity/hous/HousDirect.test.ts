...
describe('HousDirect', async () => {
  ...
  test('direct-load-hous', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-hous', setup.live)) return
  ...
  test('direct-list-hous', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-hous', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ANAPIOFICEANDFIRE_TEST_HOUS_ENTID']
