...
//     (\`new AnapioficeandfireSDK(...)\` / \`AnapioficeandfireSDK.test(...)\` become
//     \`AnapioficeandfireSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AnapioficeandfireSDK'
...
const TEST_SEED = {"entity":{"book":{"test01":{"id":"test01"}},"character":{"test01":{"id":"test01"}},"house":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AnapioficeandfireSDK(...)\` and any
// \`AnapioficeandfireSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AnapioficeandfireSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AnapioficeandfireSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
