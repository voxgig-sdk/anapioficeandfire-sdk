...
func TestHousDirect(t *testing.T) {
	t.Run("direct-list-hous", func(t *testing.T) {
		setup := housDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-hous", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-hous", func(t *testing.T) {
		setup := housDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-hous", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type housDirectSetupResult struct {
	client *sdk.AnapioficeandfireSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func housDirectSetup(mockres any) *housDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"ANAPIOFICEANDFIRE_TEST_HOUS_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["ANAPIOFICEANDFIRE_TEST_HOUS_ENTID"]; ok {
		...
		return &housDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &housDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
