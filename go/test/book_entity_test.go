...
func TestBookEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ANAPIOFICEANDFIRE_TEST_BOOK_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func bookBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ANAPIOFICEANDFIRE_TEST_BOOK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ANAPIOFICEANDFIRE_TEST_BOOK_ENTID"])
