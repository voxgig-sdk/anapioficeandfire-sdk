...
import (
	...
	omni "github.com/voxgig-sdk/anapioficeandfire-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AnapioficeandfireError); is {
				out := map[string]any{
					"name":    "AnapioficeandfireError",
...
func omniCtx(arg any, client *sdk.AnapioficeandfireSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AnapioficeandfireSDK); is {
			client = live
