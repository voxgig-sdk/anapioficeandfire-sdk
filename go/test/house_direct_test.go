...
func TestHouseDirect(t *testing.T) {
	t.Run("direct-list-house", func(t *testing.T) {
		setup := houseDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-house", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-house", func(t *testing.T) {
		setup := houseDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-house", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type houseDirectSetupResult struct {
...
func houseDirectSetup(mockres any) *houseDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["ANAPIOFICEANDFIRE_TEST_HOUSE_ENTID"]; ok {
		...
		return &houseDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &houseDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
